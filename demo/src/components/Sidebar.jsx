...
function SmartMenu({ onOpen, children }) {
  ...
  return (
    <div ref={rowRef}
      className={`rounded-lg transition-colors
        ...
        ${active && !isEditing ? 'bg-blue-50 dark:bg-blue-900/30' : ''}
    ...
    >
      {isEditing ? (
        ...
        >
          ...
          {hovered && (
            <SmartMenu onOpen={() => setMenuState('main')}>
              {(close) => {
...
function CategoryItem({
  ...
  const handleAddProject = () => {
    ...
    actions.createProject({ id: newId, categoryId: category.id, name: '新建项目', color: '#3b82f6' });
  ...
  return (
    <div>
      ...
      <div
        className={`flex items-center gap-1.5 px-2 py-2 rounded-lg transition-colors
          ${isEditing
            ...
            : 'cursor-pointer hover:bg-slate-200/60 dark:hover:bg-slate-700/60'}`}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
      ...
      >
        {/* Chevron */}
        <svg
          className={`w-3 h-3 text-slate-400 transition-transform flex-shrink-0 ${category.collapsed ? '-rotate-90' : ''}`}
        ...
        {/* Folder icon */}
        <span className="flex-shrink-0" style={{ color: category.color }}><FolderIcon /></span>
        ...
        {isEditing ? (
          ...
          <span className="flex-1 text-xs font-semibold text-slate-600 dark:text-slate-300 tracking-wide uppercase select-none truncate">
            {category.name}
          </span>
        )}
...
export default function Sidebar() {
  ...
  const handleNewCategory = () => {
    ...
    actions.createCategory({ id: newId, name: '新建分类', color: '#64748b' });
    setEditingCategoryId(newId);
  };

  return (
    <aside className="w-56 flex-shrink-0 flex flex-col h-full bg-slate-50 dark:bg-slate-900
