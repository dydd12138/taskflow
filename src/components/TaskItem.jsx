...
export default function TaskItem({
  ...
  return (
    <div className={`group border-b border-slate-100 dark:border-slate-700/50 last:border-b-0
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {task.notes && !task.completed && (
            <span className="block text-xs text-slate-400 dark:text-slate-500 truncate leading-tight mt-0.5">
              {task.notes}
            </span>
          )}
        </div>
      ...
      {/* Expanded drawer */}
      {isExpanded && (
        <TaskDrawer task={task} onClose={() => setExpandedId(null)} />
      )}
    </div>
  );
}
...
function TaskDrawer({ task, onClose }) {
  ...
  useEffect(() => {
    // Delay until after the 200ms slide-in animation so scrollIntoView sees the real height
  ...
  return (
    <div ref={drawerRef} className="drawer-open border-t-2 border-t-blue-500
        ...
        {timeType === 'due' ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <DrawerAllDayToggle value={dueAllDay} onChange={setDueAllDay} />
            </div>
            ...
            />
            {!dueAllDay && (
              <DrawerHourPicker label="截止时间" value={dueHour} onChange={setDueHour} />
            )}
          </div>
        ) : (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <DrawerAllDayToggle value={rangeAllDay} onChange={setRangeAllDay} />
            </div>
            ...
            {!rangeAllDay && (
              <div className="flex gap-2">
                <DrawerHourPicker label="开始" value={rangeStartHour} onChange={setRangeStartHour} />
                <DrawerHourPicker label="结束" value={rangeEndHour}   onChange={setRangeEndHour} />
              </div>
            )}
          </div>
        )}
