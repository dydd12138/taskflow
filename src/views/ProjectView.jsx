...
export default function ProjectView({ projectId }) {
  ...
  // Fetch note when switching to notes tab
  useEffect(() => {
    if (activeTab === 'notes') {
      actions.fetchNote(projectId);
    }
  }, [activeTab, projectId]);
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-6 pt-5 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          {/* Tab switcher — -mb-px makes each button's border-b-2 overlap the header border */}
          <div className="flex items-center -mb-px">
            {[['tasks', '任务视图'], ['notes', '笔记']].map(([tab, label]) => (
              <button
                key={tab}
      ...
      <div className="flex flex-1 min-h-0">
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === 'tasks' && (
            <div className="flex flex-col h-full">
              {/* Task toolbar */}
              <div className="flex items-center gap-2 px-6 py-2.5 border-b border-slate-100 dark:border-slate-700 flex-shrink-0">
                {/* Filter toggle */}
          ...
          {activeTab === 'notes' && (
            state.noteLoading ? (
              ...
              <NoteEditor
                ...
                onSave={(content) => actions.saveNote(projectId, content)}
